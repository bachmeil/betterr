...
#undef is_block_matrix
#define is_block_matrix(m) (m->info == (struct matrix_info_ *) INFO_INVALID)

#define is_one_by_one(m) (m->rows == 1 && m->cols == 1)

#define no_metadata(m) (m->info == NULL || is_block_matrix(m))

int real_invert_symmetric_matrix (gretl_matrix *a,
                                         int symmcheck,
					 int preserve,
					 double *ldet);
...
int math_err_check (const char *msg, int errnum)
{
#ifdef HAVE_FENV_H
    int err = E_DATA;
...
gretl_matrix_block *gretl_matrix_block_new (gretl_matrix **pm, ...)
{
    ...
    //~ for (i=0; i<B->n; i++) {
        ...
        //~ if (B->matrix[i] == NULL) {
            ...
            //~ return NULL;
        //~ }
        //~ B->matrix[i]->info = (struct matrix_info_ *) INFO_INVALID;
        //~ B->matrix[i]->info = (struct matrix_info_ *) INFO_INVALID;
        //~ B->matrix[i]->val = NULL;
        //~ B->matrix[i]->z = NULL;
...
gretl_matrix *gretl_matrix_copy_tmp (const gretl_matrix *a)
{
...
void maybe_preserve_names (gretl_matrix *targ,
                                  const gretl_matrix *src,
                                  NameFlags flags,
...
int real_gretl_matrix_is_symmetric (const gretl_matrix *m,
                                           int verbose)
{
    double x, y, ad, rd;
    int i, j;
...
double det_22 (const double *a, int *err)
{
    return a[0]*a[3] - a[1]*a[2];
}

double det_33 (const double *a, int *err)
{
...
int gretl_matrix_solve (gretl_matrix *a, gretl_matrix *b)
{
...
double gretl_general_matrix_rcond (const gretl_matrix *A,
                                          int *err)
{
    gretl_matrix *a = NULL;
    char norm = '1';
...
{
    return get_extreme_eigenvalue((struct gretl_matrix_ *) m, 0, err);
}
...
{
    return get_extreme_eigenvalue((struct gretl_matrix_ *) m, 1, err);
}
...
// Needs to be removed if gretl_cmatrix.c compiles
...
int get_ols_vcv (gretl_matrix *V, double *s2)
{
...
int ok_xy_count (const double *x,
			const double *y,
			int n, int *err)
{
    int i, n_ok = 0;
...
int libset_get_bool (SetKey key)
{
    /* global specials */
    if (key == R_FUNCTIONS) {
	return globalvars.R_functions;
    } else if (key == R_LIB) {
	return globalvars.R_lib;
    } else if (key == USE_DCMT) {
        return gretl_rand_get_dcmt();
    } else if (key == LOGSTAMP) {
	return globalvars.logstamp;
    }
